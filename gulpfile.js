...
const config = {
  libraryName: 'ngx-validate',
  unscopedLibraryName: 'ngx-validate',
  allSrc: 'src/**/*',
  allTs: 'src/**/!(*.spec).ts',
...
gulp.task('build:demo', () => {
  return execDemoCmd(`build --preserve-symlinks --prod --base-href /ngx-validate/ --deploy-url /ngx-validate/`, { cwd: `${config.demoDir}`});
});
...
gulp.task('build:demo-ssr', () => {
  return execDemoCmd(`build --preserve-symlinks --prod`, { cwd: `${config.demoDir}`})
    .then(() => execDemoCmd(`run ngx-validate-demo:server`, { cwd: `${config.demoDir}` }))
